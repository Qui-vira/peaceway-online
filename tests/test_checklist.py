...
async def _admin(db_session, tid, name, role_key, active=True):
    a = AdminUser(
    ...
    db_session.add(AdminRoleAssignment(admin_id=a.id, role_key=role_key))
    await db_session.flush()
    return a


async def _template(db_session, role_key, text="Do the thing", cadence="daily", weekday=None):
...
async def test_generate_is_idempotent(db_session):
    await _template(db_session, "dispatcher")
    await _admin(db_session, 1, "Rider Ade", "dispatcher")

    first = await checklist.generate_instances(db_session, TODAY)
    assert len(first) == 1

    second = await checklist.generate_instances(db_session, TODAY)
    assert second == []  # nothing new on re-run

    rows = (await db_session.execute(select(ChecklistInstance))).scalars().all()
    assert len(rows) == 1  # exactly one pending row per (template, assignee)


async def test_generate_skips_inactive_admins_and_weekly_off_day(db_session):
    await _template(db_session, "finance", cadence="weekly", weekday=2)  # Wednesday only
    await _admin(db_session, 2, "Fin Bola", "finance")
    await _admin(db_session, 3, "Old Staff", "finance", active=False)

    # TODAY is Monday -> the Wednesday weekly template is not due.
    assert await checklist.generate_instances(db_session, TODAY) == []
    # Wednesday -> due, but only for the active admin.
    created = await checklist.generate_instances(db_session, TODAY + timedelta(days=2))
    assert len(created) == 1


# ── completion (append-only + audit + ownership) ──────────────────────────────

async def test_complete_appends_row_and_audit(db_session):
    await _template(db_session, "dispatcher")
    admin = await _admin(db_session, 1, "Rider Ade", "dispatcher")
    [inst] = await checklist.generate_instances(db_session, TODAY)

    new = await checklist.complete_item(db_session, inst.id, admin.id, "done")
    ...
    audit = (
        await db_session.execute(select(AuditLog).where(AuditLog.action == "checklist_done"))
    ).scalar_one()
    assert audit.actor_id == admin.id
    assert audit.entity == "checklist_instance"


async def test_skip_records_reason(db_session):
    await _template(db_session, "dispatcher")
    admin = await _admin(db_session, 1, "Rider Ade", "dispatcher")
    [inst] = await checklist.generate_instances(db_session, TODAY)

    new = await checklist.complete_item(db_session, inst.id, admin.id, "skipped", skip_reason="bike broke")
    assert new.status == "skipped"
    assert new.skip_reason == "bike broke"


async def test_cannot_complete_another_persons_item(db_session):
    await _template(db_session, "dispatcher")
    owner = await _admin(db_session, 1, "Rider Ade", "dispatcher")
    other = await _admin(db_session, 2, "Rider Kunle", "dispatcher")
    instances = await checklist.generate_instances(db_session, TODAY)
    ade_item = next(i for i in instances if i.assigned_user_id == owner.id)

    # Kunle tries to tick Ade's item -> guarded, no write.
    assert await checklist.complete_item(db_session, ade_item.id, other.id, "done") is None
    rows = (
...
async def test_pa_cannot_complete_others_item(db_session):
    """A PA has view rights but may only complete her OWN items."""
    await _template(db_session, "dispatcher")
    rider = await _admin(db_session, 1, "Rider Ade", "dispatcher")
    pa = await _admin(db_session, 9, "PA Zainab", "pa")
    [rider_item] = await checklist.generate_instances(db_session, TODAY)

    assert await checklist.complete_item(db_session, rider_item.id, pa.id, "done") is None


async def test_db_check_blocks_foreign_completion_on_raw_insert(db_session):
    ...
    db_session.add(
        ChecklistInstance(
            template_id=(await db_session.execute(select(ChecklistTemplate.id))).scalar_one(),
...
async def test_aggregate_counts_only(db_session):
    await _template(db_session, "dispatcher", text="Item A")
    await _template(db_session, "dispatcher", text="Item B")
    a = await _admin(db_session, 1, "Ade", "dispatcher")
    b = await _admin(db_session, 2, "Bola", "dispatcher")
    instances = await checklist.generate_instances(db_session, TODAY)  # 4 rows
    # Ade completes one item.
    ade_item = next(i for i in instances if i.assigned_user_id == a.id)
    await checklist.complete_item(db_session, ade_item.id, a.id, "done")

    agg = await checklist.aggregate_status(db_session, TODAY)
    assert agg == {
        "done": 1, "total": 4, "outstanding_items": 3, "people_with_outstanding": 2,
    }


async def test_per_person_status(db_session):
    await _template(db_session, "dispatcher", text="Item A")
    a = await _admin(db_session, 1, "Ade", "dispatcher")
    [item] = await checklist.generate_instances(db_session, TODAY)
    await checklist.complete_item(db_session, item.id, a.id, "done")

    people = await checklist.per_person_status(db_session, TODAY)
    assert people == [{"name": "Ade", "done": 1, "total": 1, "skipped": []}]


# ── decisions / Monday prep ──────────────────────────────────────────────────

async def test_open_decisions_flags_overdue(db_session):
    owner = await _admin(db_session, 5, "Owner O", "system_owner")
    db_session.add(MeetingDecision(decision_text="Order new bags", owner_user_id=owner.id,
                                due_date=TODAY - timedelta(days=8), status="open"))
    db_session.add(MeetingDecision(decision_text="Fresh task", owner_user_id=owner.id,
                                due_date=TODAY - timedelta(days=1), status="open"))
    await db_session.flush()

    decisions = await checklist.open_decisions(db_session, TODAY)
...
async def test_admins_with_pending_excludes_completed(db_session):
    await _template(db_session, "dispatcher", text="Item A")
    a = await _admin(db_session, 1, "Ade", "dispatcher")
    b = await _admin(db_session, 2, "Bola", "dispatcher")
    instances = await checklist.generate_instances(db_session, TODAY)
    ...
    pending = await checklist.admins_with_pending(db_session, TODAY)
    assert pending == [(b.id, 1)]


async def test_nudge_idempotency_via_audit(db_session):
    """A recorded checklist_nudge_sent row removes that admin from the next run's targets."""
    from app.models import AuditLog

    await _template(db_session, "dispatcher")
    a = await _admin(db_session, 1, "Ade", "dispatcher")
    await checklist.generate_instances(db_session, TODAY)

    assert await checklist.nudged_admin_ids(db_session, TODAY) == set()
    # Simulate the job claiming the nudge.
    db_session.add(AuditLog(action="checklist_nudge_sent", entity="admin_user",
                         entity_id=str(a.id), detail={"due_date": TODAY.isoformat(), "outstanding": 1}))
    await db_session.flush()

    assert await checklist.nudged_admin_ids(db_session, TODAY) == {a.id}
    # A different day is unaffected.
    assert await checklist.nudged_admin_ids(db_session, TODAY + timedelta(days=1)) == set()
