...
- After modifying code, keep the graph current with `python scripts/graphify_ast_serial.py`
  (AST-only, no API cost). Do **not** run a plain `graphify update .` on this machine: its
  parallel worker pool crashes here and silently drops files, so affected sources vanish from
  the graph with only a warning. See the known-issue section in CLAUDE.md before rebuilding.
